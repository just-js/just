const { fs, net, sys, SystemError } = just
...
function getStats (stat) {
  ...
  st.flags = stat[10]
  st.st_gen = stat[11] // ?
  st.accessed = { tv_sec: stat[12], tv_nsec: stat[13] }
  st.modified = { tv_sec: stat[14], tv_nsec: stat[15] }
  st.created = { tv_sec: stat[16], tv_nsec: stat[17] }
  st.permissions = {
    user: {
...
function readFileBytes (path, flags = fs.O_RDONLY) {
  ...
  if (fd < 0) throw new SystemError(`File ${path} does not exist`)
  let r = fs.fstat(fd, stat)
  if (r < 0) throw new SystemError(`Error getting stats for ${path}`)
  const size = Number(stat[7])
  const buf = new ArrayBuffer(size)
  let off = 0
  let len = net.read(fd, buf, off, buf.byteLength)
  while (len > 0) {
    off += len
    if (off === size) break
    len = net.read(fd, buf, off, buf.byteLength)
  }
  off += len
  r = net.close(fd)
  if (len < 0) {
    const errno = sys.errno()
    throw new SystemError('Error Reading File')
  }
  if (off < size) {
    throw new Error(`Size Mismatch: size: ${size}, read: ${off}`)
  }
  if (r < 0) throw new SystemError('Error Closing File')
  return buf
}
...
function copyFile (src, dest) {
  ...
  let bytes = net.read(source.fd, buf, 0, buf.byteLength)
  while (bytes > 0) {
    net.write(destination.fd, buf, bytes, 0)
    bytes = net.read(source.fd, buf, 0, buf.byteLength)
  }
  net.close(source.fd)
...
function getModified (fd) {
  if (fs.fstat(fd, stat) < 0) return -1
  // TODO: remove dependency on Date
  return new Date(Number(stat[14]) * 1000)
}
