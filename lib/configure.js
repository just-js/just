...
function getModule (fileName, static, libName) {
  ...
  if (json) {
    ...
    entry.lib = static ? (json.staticLib || json.lib) : json.lib
  } else {
    if (!moduleCache[libName]) {
      if (!entry.obj) entry.obj = []
      entry.obj.push(`modules/${libName}/${fileName}.o`)
...
function parse (fileName, type = 'script', static = false) {
        ...
        if (expr.type === 'CallExpression') {
          ...
          if (libName) {
            if (fileName.type === 'Literal' && libName.type === 'Literal') {
              getModule(fileName.value, static, libName.value)
            }
          } else {
            if (fileName.type === 'Literal') {
              getModule(fileName.value, static)
            }
          }
        }
      }
...
function run (scriptName, opts = {}) {
  ...
  const { index, libs, modules, natives } = parse(scriptName, 'script', config.static)
