...
const defaultOpts = {
  host: '127.0.0.1',
  port: 9222,
  favicon: 'https://just.billywhizz.io/favicon.ico',
  onReady: () => {}
}
...
function createInspector (opts = defaultOpts) {
  ...
  function sha1 (str) {
    ...
    return source.readString(encode.base64Encode(dest, source, just.sha1.hash(source, dest)))
  }
  ...
  function startWebSocket (request) {
    ...
    parser.onMessage = header => {
      ...
      try {
        ...
        //just.error(`${AG}inspector.parse.error${AD}\n${str}`)
  ...
  function getJSON () {
    ...
    if (just.args.length > 1) {
      fileName = just.args[1].split('/').slice(-1)[0]
    }
    res.push('HTTP/1.1 200 OK')
    res.push('Content-Type: application/json; charset=UTF-8')
    res.push('Cache-Control: no-cache')
  ...
  function onSocketEvent (fd, event) {
    ...
    const client = clients[fd]
    const { buffer } = client.http
    const bytes = net.recv(fd, buffer, 0, buffer.byteLength)
    if (bytes > 0) {
      if (client.websocket) {
        ...
        if (count > 0) {
          for (let i = 0; i < count; i++) {
            ...
            if (request.method !== 'GET') {
              net.sendString(fd, getNotFound())
              continue
            }
            request.fd = fd
            if (request.url === '/json' || request.url === '/json/list') {
              net.sendString(fd, getJSON())
              continue
            }
            if (request.url === '/json/version') {
              net.sendString(fd, getJSONVersion())
              continue
            }
  ...
  global.receive = message => {
    //just.error(`${AM}global.receive${AD}\n${message}`)
    const buf = ws.createMessage(message)
    net.send(clientfd, buf, buf.byteLength, 0)
  }
