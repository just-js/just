...
async function run (config = {}, { cleanall = false, clean = false, dump = false, silent = false }) {
  ...
  for (const module of modules) {
    ...
    if (!isFile(`./modules/${module.name}/${module.name}.o`)) {
      ...
      const r = await make({ silent }, `MODULE=${module.name}`, `CFLAGS=${CFLAGS}`, `LFLAGS=${LFLAGS}`, moduleBuild)
      const status = r ? `${AR}failed${AD}` : `${AG}complete${AD}`
      // TODO: remove dependency on Date
      print(`${status} in ${AY}${Math.floor((Date.now() - start) / 10) / 100}${AD} sec`)
    }
  ...
  const r = await make({ silent }, `FLAGS=${flags}`, `EMBEDS=${config.EMBEDS}`, `MAIN=${main}`, `RELEASE=${version}`, `LIBS=${config.LIBS}`, `MODULES=${config.MODULES}`, `TARGET=${target}`, `LIB=${config.LIB}`, `CFLAGS=${CFLAGS}`, `LFLAGS=${LFLAGS}`, build)
  const status = r ? `${AR}failed${AD}` : `${AG}complete${AD}`
  // TODO: remove dependency on Date
  print(`${status} in ${AY}${Math.floor((Date.now() - start) / 10) / 100}${AD} sec`)
  if (config.target !== 'just') {
