...
function readBytes (fileName) {
  const buf = readFileBytes(fileName)
  ...
  return { bytes, size: buf.byteLength }
}

function sourceToBinary (main, index) {
  const b = readBytes(main)
  if (index) {
    const i = readBytes(index)
    return `static const char just_js[] = {\n${b.bytes.join('')}\n};\nstatic unsigned int just_js_len = ${b.size};\nstatic unsigned int _use_index = 1;\nstatic const char index_js[] = {\n${i.bytes.join('')}\n};\nstatic unsigned int index_js_len = ${i.size};\n`
  }
  return `static const char just_js[] = {\n${b.bytes.join('')}\n};\nstatic unsigned int just_js_len = ${b.size};\nstatic unsigned int _use_index = 0;\nstatic const char* index_js = NULL;\nstatic unsigned int index_js_len = 0;\n`
}

function generateBuiltins (libs, embeds, modules) {
  const lines = []
...
async function run (config = {}, ...args) {
  ...
  let src = sourceToBinary(main, index)
  src += generateBuiltins(libs, embeds, modules)
