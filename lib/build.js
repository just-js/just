...
function make (justDir, opts, ...args) {
  const { silent } = opts
  if (silent) {
    const process = launch('make', ['C=gcc', 'CC=g++', ...args], justDir)
...
async function run (config = {}, { clean = false, dump = false, silent = false }) {
  ...
  if (!isFile('deps/v8/libv8_monolith.a')) {
    ...
    const r = await make(justDir, { silent }, 'deps/v8/libv8_monolith.a')
  ...
  if (modules.length && !isDir('modules')) {
    ...
    const r = await make(justDir, { silent }, 'modules')
    const status = r ? `${AR}failed${AD}` : `${AG}complete${AD}`
    just.print(`${status} in ${AY}${Math.floor((Date.now() - start) / 10) / 100}${AD} sec`)
  }
  if (clean) {
    just.print(`${AG}clean ${target}${AD} `, false)
    const r = await make(justDir, { silent }, `TARGET=${target}`, 'clean')
    const status = r ? `${AR}failed${AD}` : `${AG}complete${AD}`
    just.print(`${status} in ${AY}${Math.floor((Date.now() - start) / 10) / 100}${AD} sec`)
  }
  for (const module of modules) {
    if (clean) {
      ...
      const r = await make(justDir, { silent }, '-C', `modules/${module.name}`, 'clean')
      const status = r ? `${AR}failed${AD}` : `${AG}complete${AD}`
      just.print(`${status} in ${AY}${Math.floor((Date.now() - start) / 10) / 100}${AD} sec`)
    }
    if (!isFile(`./modules/${module.name}/${module.name}.o`)) {
      just.print(`${AG}build modules/${module.name}${AD} `, false)
      const r = await make(justDir, { silent }, `MODULE=${module.name}`, moduleBuild)
      const status = r ? `${AR}failed${AD}` : `${AG}complete${AD}`
      just.print(`${status} in ${AY}${Math.floor((Date.now() - start) / 10) / 100}${AD} sec`)
    }
  }
  just.print(`${AG}build ${target}${AD} ${AY}${version}${AD} (${main}) `, false)
  const r = await make(justDir, { silent }, `FLAGS=${flags}`, `EMBEDS=${config.EMBEDS}`, `MAIN=${main}`, `RELEASE=${version}`, `LIBS=${config.LIBS}`, `MODULES=${config.MODULES}`, `TARGET=${target}`, `LIB=${config.LIB}`, build)
  const status = r ? `${AR}failed${AD}` : `${AG}complete${AD}`
  just.print(`${status} in ${AY}${Math.floor((Date.now() - start) / 10) / 100}${AD} sec`)
}
